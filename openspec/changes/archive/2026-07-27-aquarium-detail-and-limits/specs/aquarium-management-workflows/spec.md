...
#### Scenario: Edit navigates to a full-page detail view
- **WHEN** a user clicks "Edit" on an aquarium row in the aquariums management view
- **THEN** the system navigates to a dedicated detail page for that aquarium, rather than opening an in-page sidebar/drawer, and the aquarium's editable fields are presented in that page

#### Scenario: Successful record update
- **WHEN** a user edits an existing record's fields on the aquarium detail page and confirms changes
- **THEN** the system persists changes through the API and reflects the updated data without requiring a separate destructive-action prompt
...
#### Scenario: Add salinity measurement to aquarium workflow
- **WHEN** a user submits a valid salinity measurement for an aquarium
- **THEN** the system stores the measurement through the dedicated measurement API from the dedicated measurements section and makes it available in the aquarium measurement history view

#### Scenario: View salinity measurements within management workflow
- **WHEN** a user navigates to the aquarium measurements workflow
- **THEN** the system shows available historical measurements and supports trend-oriented review for that aquarium
